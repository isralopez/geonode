...
def get_related_documents(resource):
    if isinstance(resource, Layer) or isinstance(resource, Map):
        ...
        return Project.objects.filter(content_type=ct, object_id=resource.pk)
    else:
        return None
...
def update_documents_extent(sender, **kwargs):
    ...
    for document in Project.objects.filter(content_type=ctype, object_id=sender.id):
        document.save()

signals.pre_save.connect(pre_save_document, sender=Project)
signals.post_save.connect(create_thumbnail, sender=Project)
signals.post_save.connect(resourcebase_post_save, sender=Project)
